...
//-----------------------------------------------------------------------------
bool vtkSMCPStateLoader::SkipProxy(const char* xml_group, const char* xml_name)
{
  if (xml_group && xml_name)
    {
    if (!strcmp(xml_group, "animation") && !strcmp(xml_name, "AnimationScene"))
      {
      return true;
      }

    if (!strcmp(xml_group, "misc") && !strcmp(xml_name, "TimeKeeper"))
      {
      return true;
      }
    }

  return this->Superclass::SkipProxy(xml_group, xml_name);
}
